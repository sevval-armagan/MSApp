...
class MovieDetailsVC: UIViewController{
    ...
    private var MCM = [MoviesCastModel]()
    ...
    func createStackViewButton (i:Int){
        ...
        if(button.tag == i){
            button.setTitle(MDM[0].genres![i].name, for: .normal)
        }
    }
    //--------------------------------------------------------------------------------------------------//
    ...
    func createCastImage (i:Int){
        ...
        if ((MCM[0].cast![i].profile_path?.isEmpty) != nil){
            createCastImage.loadImageAsync(with: "https://image.tmdb.org/t/p/original" + MCM[0].cast![i].profile_path!, completed: {})
        ...
        imageLabel.text = firstLetterName(a: MCM[0].cast![i].name!)
        ...
        nameLabel.text = MCM[0].cast![i].name!
    }
    ...
    func atamalarYapilacak(){
        ...
        self.voteAvarageCountLabel.text =  String(format: "%.1f", MDM[0].vote_average!)
        posterImage.loadImageAsync(with: "https://image.tmdb.org/t/p/original" + MDM[0].poster_path!, completed: {})
    
        if let asdas3d = MDM[0].overview{
        ...
        if (MDM[0].runtime == nil){
            ...
            self.runTimeCountLabel.text = String(MDM[0].runtime!)
        }
        
        genresCount = MDM[0].genres!.count
        castCount = MCM[0].cast!.count
        
        
        
        //Sayfa Kapanmadan RemoveAll()
        moviesDetailsViewModel.moviesDetailsArray.removeAll()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        moviesDetailsViewModel.getDataMovieDetails(id: movieID, completed: {})
